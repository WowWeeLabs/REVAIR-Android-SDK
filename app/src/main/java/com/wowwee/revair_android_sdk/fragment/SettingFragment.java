...
public class SettingFragment extends REVAirBaseFragment {
    Handler handler;
    @Override
                ...
                if (REVAirFinder.getInstance().getREVAirRobotConnectedList().size() > 0) {
                    ...
                    switch (position) {
                        case 0:
                            ...
                            break;
                        case 1:
                            robot.revairFirmwareVersion();
                            break;
                        case 2:
                            robot.revairGetBatteryLevel();
                            break;
                        case 3:
                            ...
                            builder.setNegativeButton("Disable", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                    robot.revairSetWallDetectionMode(false);
                                    dialog.cancel();
                                }
                            });
    ...
    @Override
    public void revairDidCalibrate(REVAir revAirRobot, final boolean b) {
    ...
    @Override
    public void revairDidResetCalibration(REVAir var1, final boolean b) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                AlertDialog alertDialog = new AlertDialog.Builder(getActivity()).create();
    ...
    @Override
    public void revairDidReceiveFirmwareVersion(REVAir revAirRobot, final int i, final int i1) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                AlertDialog alertDialog = new AlertDialog.Builder(getActivity()).create();
    ...
    @Override
    public void revairDidReceiveBatteryInfo(REVAir revAirRobot, final float i) {
        handler.post(new Runnable() {
            @Override
            public void run() {
                AlertDialog alertDialog = new AlertDialog.Builder(getActivity()).create();
